...
- Escaneo automático real con **`<video>` propio + ZXing directo**: en vez de la clase `Html5Qrcode` (su sampler se liga al aspect del elemento y distorsiona con regiones verticales), `ScanPage` hace `getUserMedia` con `facingMode: {ideal:'environment'}` + `width/height: {ideal: 1920/1080, max: 1920/1080}` (calidad máxima; **sin** `aspectRatio` ideal — forzaba resoluciones raras y bajas) y renderiza el `<video>` en **pantalla completa con `object-fit: cover`** (soporta streams retrato y paisaje). El loop de decodificación (`setInterval` ~125ms, fps 8) calcula el **sub-rectángulo visible del video** (math de cover crop: `scale = max(bW/vW, bH/vH)`, `sx/sy = (drawn - box)/2/scale`) y dibuja SOLO ese recorte en un canvas offscreen (cap lado largo 600px — un canvas 615x1080 + TRY_HARDER síncrono bloqueaba el main thread y dejaba el scan a ~2fps) → decode propio con ZXing (`src/lib/zxing/zxing-js.umd` — vendido del antiguo paquete `html5-qrcode`, que ya no es dependencia; `vite.config.ts` lo incluye en `commonjsOptions` y `.oxlintrc.json` lo ignora; `MultiFormatReader` con `TRY_HARDER: true` + `POSSIBLE_FORMATS` limitados). Decodifica QR/EAN/UPC/Code128/Code39 y funciona en todos los navegadores (incl. iOS Safari, donde `BarcodeDetector` no existe o falla silenciosamente). La vista previa y el sampler muestran exactamente la misma imagen — sin distorsión.
