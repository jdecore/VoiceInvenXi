...
- Escaneo automático real con **`<video>` propio + ZXing directo** (restaurado: Quagga2 daba problemas de foco/precisión en móvil): en vez de la clase `Html5Qrcode` (su sampler se liga al aspect del elemento y distorsiona con regiones verticales), `ScanPage` hace `getUserMedia` con `facingMode: {ideal:'environment'}` probando resoluciones en orden descendente (1920x1080 → 1280x720 → default; algunas cámaras rechazan `max: 1920` con OverconstrainedError) y renderiza el `<video>` en **pantalla completa con `object-fit: cover`** (soporta streams retrato y paisaje). El loop de decodificación (`setInterval` 125ms, ~8fps) calcula el **sub-rectángulo visible del video** (math de cover crop: `scale = max(bW/vW, bH/vH)`, `sx/sy = (drawn - box)/2/scale`) y dibuja SOLO ese recorte en un canvas offscreen (cap lado largo 600px — un canvas 615x1080 + TRY_HARDER síncrono bloqueaba el main thread y dejaba el scan a ~2fps) → decode propio con ZXing (`src/lib/zxing/zxing-js.umd` — vendido del antiguo paquete `html5-qrcode` y convertido de UMD a ESM real; `vite.config.ts` lo incluye en `commonjsOptions` y `.oxlintrc.json` lo ignora; `MultiFormatReader` con `TRY_HARDER: true` + `POSSIBLE_FORMATS` limitados se reutiliza con `decodeWithState`). Decodifica QR/EAN/UPC/Code128/Code39 y funciona en todos los navegadores (incl. iOS Safari, donde `BarcodeDetector` no existe o falla silenciosamente). La vista previa y el sampler muestran exactamente la misma imagen — sin distorsión. Restaurado desde `c585704` (último commit bueno antes de migrar a Quagga).
- Beep de escaneo con Web Audio (`src/lib/beep.ts`, `playScanBeep`): el `AudioContext` se desbloquea en la primera interacción del usuario (`pointerdown/touchstart/keydown` una sola vez) — sin esto, iOS/Chrome móvil lo mantienen `suspended` y el beep (fuera de gesto de usuario) no suena. + `hapticSuccess()` al detectar un código
