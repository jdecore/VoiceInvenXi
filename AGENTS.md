...
- Escaneo automático real con **`<video>` propio + ZXing directo**: en vez de la clase `Html5Qrcode` (su sampler se liga al aspect del elemento y distorsiona con regiones verticales), `ScanPage` hace `getUserMedia` con `facingMode: {ideal:'environment'}` + `aspectRatio: {ideal: retrato}` y renderiza el `<video>` en **pantalla completa con `object-fit: cover`** (soporta streams retrato y paisaje). El loop de decodificación (`setInterval` ~125ms, fps 8) calcula el **sub-rectángulo visible del video** (math de cover crop: `scale = max(bW/vW, bH/vH)`, `sx/sy = (drawn - box)/2/scale`) y dibuja SOLO ese recorte en un canvas offscreen (`@ 640px max` para perf) → `ZXingHtml5QrcodeDecoder.decodeAsync()` (imports deep: `html5-qrcode/esm/zxing-html5-qrcode-decoder` + `html5-qrcode/esm/core`). Decodifica QR/EAN/UPC/Code128/Code39 y funciona en todos los navegadores (incl. iOS Safari, donde `BarcodeDetector` no existe o falla silenciosamente). La vista previa y el sampler muestran exactamente la misma imagen — sin distorsión.
