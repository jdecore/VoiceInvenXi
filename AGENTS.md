...
- Escaneo automático con **`html5-qrcode` (`Html5Qrcode` clase, v2.3.8 — restaurado: ZXing directo y Quagga2 daban problemas de foco/precisión)**. `ScanPage` crea `new Html5Qrcode('scan-region', false)` y llama `start({facingMode:'environment'}, {fps:8}, onSuccess)`. La librería renderiza su propio `<video>`+`<canvas>` dentro de `#scan-region` (strip central `aspect-video` sin distorsión, ver `cd7a235`). El overlay de recuadro (esquinas `corner-pulse`, línea `scan-line`, `ring` naranja) es solo visual — `html5-qrcode` decodifica internamente con ZXing (QR/EAN/UPC/Code128/Code39). Manejo de crash en navegación: `stop().catch→clear()` ordenado para evitar "Cannot clear while scan is ongoing" y soporte `StrictMode`. Ver `src/pages/ScanPage.tsx:26-60`.
