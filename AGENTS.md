...
- Escaneo automático real con **`<video>` propio + Quagga2 (`@ericblade/quagga2`)**: en vez de la clase `Html5Qrcode` (su sampler se liga al aspect del elemento y distorsiona con regiones verticales), `ScanPage` hace `getUserMedia` con `facingMode: {ideal:'environment'}` probando resoluciones en orden descendente (1920x1080 → 1280x720 → default; algunas cámaras rechazan `max: 1920` con OverconstrainedError) y renderiza el `<video>` en **pantalla completa con `object-fit: cover`** (soporta streams retrato y paisaje). El loop de decodificación (`setInterval` 150ms, ~6-7fps) calcula el **sub-rectángulo visible del video** (math de cover crop: `scale = max(bW/vW, bH/vH)`, `sx/sy = (drawn - box)/2/scale`) y dibuja SOLO ese recorte en un canvas offscreen (cap lado largo 600px — un canvas 615x1080 bloqueaba el main thread y dejaba el scan a ~2fps) → decode con `Quagga.decodeSingle` (`locate: true`, `patchSize: medium`, `halfSample: true`, readers `ean/ean_8/upc/code_128/code_39`; sin QR — Quagga es 1D). Funciona en todos los navegadores (incl. iOS Safari, donde `BarcodeDetector` no existe). La vista previa y el sampler muestran exactamente la misma imagen — sin distorsión. El `handleScan` usa `handleScanRef` para evitar dependencia circular con `scanFrame`/interval y `isScanningRef` (`useRef(false)` sin efecto de sync sobrante) como lock.
